# For Agents — Screen Wakelock Detector

## Milestone autopilot (M0–M15)

See also [`.cursor/AGENTS.md`](../.cursor/AGENTS.md).

- **Run until milestone complete** — do not stop after one task
- **Smoke before archive:** `scripts/smoke/m{N}_smoke.sh` exit 0 recorded in `docs/GATES.md`
- **Never archive** without smoke PASS (`archive-completed-tasks.py` enforces)
- **Lead agent only** commits/pushes per milestone
- **Parallel:** spawn Task subagents for `<!-- PARALLEL -->` blocks

Production path: **`app/`** — see [`BOOTSTRAP_TEMPLATE_MAP.md`](BOOTSTRAP_TEMPLATE_MAP.md).

---

# For Agents (bootstrap reference)

## Phased Loading

SessionStart -> START_HERE.md -> CURSOR_MODES (pick mode) -> AGENTS.md -> BUILD_PLAN Sequential -> modules/android/MODULE.md -> Plan or Agent
...
Required green workflows: **Android CI** on this repo (FOSS Android app). Template repos also use **Security Scan** and **CodeQL** — override with `GITHUB_REQUIRED_WORKFLOWS=CI,Security Scan,CodeQL` when those workflows exist.
