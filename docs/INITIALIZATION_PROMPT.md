...
**Platform/Tech Stack:** Android — Kotlin, Jetpack Compose Material 3, Room, Hilt, libsu (production module `app/`)

**Purpose & Goals:** FOSS app that logs screen wake events, attributes them to apps/channels, and offers one-tap fixes. F-Droid-only distribution. No cloud, no analytics, no `INTERNET` permission.

**Distribution:** Apache-2.0 · F-Droid · GitHub Releases for signed APKs · GitLab for MRs
